    ...
    for (const auto& [key, value] : *node_t) {
        ...
        // == size ==> collection of two items [width, height], where each is specified either by integer or a string with variable name
            ...
            if (const auto* value_arr_ptr = value.as_array(); value_arr_ptr && value_arr_ptr->size() == 2) {
                ...
                m_error_source_region = value.source();
                m_error_description = "An array of two integers/strings of variable names must follow after 'size='";
                m_is_error = true;
            }
        }
...
void Parser::SetIntFromVariable(const toml::node& value, int& to_set)
{
    if (const auto* value_int_ptr = value.as_integer()) {
        ...
        if (const auto it = m_variables.find(value_str_ptr->get()); it != m_variables.end()) {
            ...
            m_error_source_region = value.source();
            m_error_description = std::format("Variable '{}' does not exist", value_str_ptr->get());
            m_is_error = true;
        }
    }
    else if (!m_is_error) {
        m_error_source_region = value.source();
        m_error_description = "Value must be specified as an integer or a string with a variable name";
        m_is_error = true;
    }
}
